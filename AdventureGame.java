...
public class AdventureGame implements Serializable {
    ...
    private boolean accessible = false; // accessibility of the game
    protected HashMap<String, AdventureObject> objectsInGame = new HashMap<>(); // An attribute to store all the objects within game with name as key
    ...
    public String interpretAction(String command){
        ...
        if (motionTable.optionExists(inputArray[0])) {
            List<Passage> directions = motionTable.getDirection();
            for (Passage direction : directions) {
                if (direction.getDirection().equalsIgnoreCase(inputArray[0])) {
                    if (direction.getIsBlocked()) {
                        if (direction.getKeyName().contains("TRIVIA")) return direction.getKeyName();}
                }
            }
     ...
     * getAccessibility
     * __________________________
     * Getter method for accessibility
     * @return accessible (synonym to command)
     */
     ...
     * toggleAccessibility
     * __________________________
     * Setter method for accessibility
     */
    public void toggleAccessibility() {
        ...
        // methods that change according to accessibility (highlight objects etc.)
    }
